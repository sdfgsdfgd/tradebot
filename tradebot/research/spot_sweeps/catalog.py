...
def _freeze_all_except(*active_dims: str) -> tuple[str, ...]:
    """Keep preset scope readable: name explored dimensions, freeze the rest."""
    active = {str(dim) for dim in active_dims}
    unknown = active - set(_COMBO_FULL_CARTESIAN_DIM_ORDER)
    if unknown:
        raise ValueError(f"Unknown combo_full dimensions: {', '.join(sorted(unknown))}")
    return tuple(dim for dim in _COMBO_FULL_CARTESIAN_DIM_ORDER if dim not in active)


_COMBO_FULL_COVERAGE_TIER_REGISTRY: dict[str, dict[str, object]] = {
    "full": {"freeze_dims": ()},
    "profile": {
        "freeze_dims": _freeze_all_except("timing_profile"),
...
_COMBO_FULL_PRESET_ALIAS_REGISTRY: dict[str, dict[str, object]] = {
    "squeeze": {
        "tier": "regime",
        "customizer": "squeeze",
        "freeze_dims": _freeze_all_except("confirm", "tod", "vol", "regime2"),
    },
    "tod_interaction": {
        "tier": "gate",
        "customizer": "tod_interaction",
        "freeze_dims": _freeze_all_except("tod", "cadence"),
    },
    ...
    "ema_atr": {
        "tier": "ema",
        "customizer": "ema_atr",
        "freeze_dims": _freeze_all_except("direction", "exit"),
    },
    "r2_atr": {
        "tier": "regime",
        "customizer": "r2_atr",
        "freeze_dims": _freeze_all_except("regime2", "exit"),
    },
    "r2_tod": {
        "tier": "regime",
        "customizer": "r2_tod",
        "freeze_dims": _freeze_all_except("tod", "regime2"),
    },
    "loosen_atr": {
        "tier": "risk",
        "customizer": "loosen_atr",
        "freeze_dims": _freeze_all_except("exit"),
    },
    ...
    "lf_shock_sniper": {
        ...
        "freeze_dims": _freeze_all_except("direction", "shock"),
    },
    "hf_timing_sniper": {
        "tier": "profile",
        "customizer": "hf_timing_sniper",
