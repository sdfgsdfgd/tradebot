...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--router-version", choices=("v2", "v3", "v4"), default="v4")
    ...
    for year in range(int(args.start_year), int(args.end_year) + 1):
        ...
        elif str(args.router_version) == "v3":
            decision = classify_climate_v3(features)
        else:
            decision = classify_climate_v4(features)
        ...
        lf2_pnl, lf2_dd, lf2_pdd = named_host_year_pdd(daily_bars, year, "lf_defensive_long_v2")
        ...
        chosen = {
            ...
            "lf_defensive_long_v2": ("lf_defensive_long_v2", lf2_pnl, lf2_dd, lf2_pdd, 0, 0.0),
        ...
        rows.append(
            {
                ...
                "lf_defensive_long_v2_pnl": float(lf2_pnl),
                "lf_defensive_long_v2_dd_pct": float(lf2_dd),
                "lf_defensive_long_v2_pdd": float(lf2_pdd),
    ...
    if str(args.router_version) == "v2":
        ...
        lines.append("- `negative_transition_bear`: otherwise -> host=`lf_defensive_long_v2` (`drawdown_kill`, `on=15%`, `off=8%`, no MA re-entry gate`) ")
    lines.append("")
    lines.append("| Year | ret | maxdd | rv | eff | dd>=10% frac | climate | buyhold | sma200 | lf_def_v1 | lf_def_v2 | hf_host | chosen | chosen pdd |")
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | --- | ---: |")
    for row in rows:
        lines.append(
            ...
            f"{row['climate_v2']} | {float(row['buyhold_pdd']):.3f} | {float(row['sma200_pdd']):.3f} | {float(row['lf_defensive_long_v1_pdd']):.3f} | {float(row['lf_defensive_long_v2_pdd']):.3f} | "
