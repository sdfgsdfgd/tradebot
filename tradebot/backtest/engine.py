...
def _spot_prepare_summary_series_pack(
    *,
    ...
    include_tick = str(getattr(cfg.strategy, "tick_gate_mode", "off") or "off").strip().lower() != "off"
...
def _spot_build_series_pack(
    *,
    ...
    loaded = (
        _spot_series_pack_persistent_get(cache_dir=cfg.backtest.cache_dir, key_hash=pack_key_hash)
        if use_persist
        else None
    )
    if loaded is not None:
        _SERIES_CACHE.set(namespace=_SPOT_SERIES_PACK_NAMESPACE, key=pack_key, value=loaded)
        return loaded

    pack = _SpotSeriesPack(
        align=_spot_exec_alignment(signal_bars, exec_bars),
        tick_series=(
            _spot_tick_gate_series(signal_bars=signal_bars, tick_bars=tick_bars, strategy=cfg.strategy)
            if include_tick
            else None
        ),
...
def _spot_branch_size_mult(*, policy: _SpotPolicyRun, entry_branch: str | None) -> float:
...
def _run_spot_backtest_exec_loop(
    ...
    *,
    ...
    policy = _SpotPolicyRun(
        lifecycle_graph=SpotPolicyGraph.from_sources(strategy=cfg.strategy, filters=None),
        lifecycle_config=SpotPolicyConfigView.from_sources(strategy=cfg.strategy, filters=None),
        sizing_graph=SpotPolicyGraph.from_sources(strategy=cfg.strategy, filters=filters),
        sizing_config=SpotPolicyConfigView.from_sources(strategy=cfg.strategy, filters=filters),
    )
    ...
    if start_exec_idx > 0:
        for warm_idx in range(int(start_exec_idx)):
            ...
            warm_is_last_bar = warm_next is None or exec_dates[int(warm_idx) + 1] != exec_dates[int(warm_idx)]
    ...
    for idx in range(int(start_exec_idx), len(exec_bars)):
        ...
        if pending_decision is not None and pending_decision.intent == "exit" and open_trades:
        ...
        if (
            ...
            if resize_leg is not None and trade_dir in ("up", "down"):
                ...
                if int(signed_target) != 0:
                    size_mult = _spot_branch_size_mult(policy=policy, entry_branch=trade.entry_branch)
