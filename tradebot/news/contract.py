"""Strict schema and persistent-state contract for the one-shot news reducer."""
...
def publication_id(snapshot: Mapping[str, object]) -> str:
    payload = {key: value for key, value in snapshot.items() if key != "publication_id"}
    return sha256(
        json.dumps(
            payload,
            ensure_ascii=False,
...
def load_news_history(path: Path) -> tuple[dict[str, object], ...]:
    """Read one append-time publication ledger without racing its writer."""
    ...
    try:
        ...
        raise NewsError(f"news history read failed: {exc}") from exc
    snapshots = []
    for line_no, line in enumerate(raw.splitlines(), 1):
        ...
        try:
            ...
            raise NewsError(f"{path}:{line_no}: invalid news snapshot JSON") from exc
        if not isinstance(snapshot, dict):
            raise NewsError(f"{path}:{line_no}: news snapshot must be an object")
        snapshots.append(snapshot)
    return tuple(snapshots)


def select_news_snapshot_at(
    snapshots: Sequence[Mapping[str, object]],
    *,
    as_of: datetime,
) -> dict[str, object] | None:
    """Return the latest publication causally visible at one decision time."""
    ...
    for index, snapshot in enumerate(snapshots):
        published_at = _parse_utc(
            snapshot.get("snapshot_as_of_utc"),
            label=f"news history row {index + 1}.snapshot_as_of_utc",
        )
        if published_at <= decision_at:
            available.append((published_at, index, snapshot))
    if not available:
        return None
    return dict(max(available, key=lambda row: (row[0], row[1]))[2])


@dataclass(frozen=True)
class NewsSignalObservation:
    """Timestamp-correct, non-authoritative view of one published asset signal."""

    symbol: str
    run_status: str
    signal_as_of_utc: str
    ...
    def as_payload(self) -> dict[str, object]:
        return {
            ...
            "signal_as_of_utc": self.signal_as_of_utc,
            "snapshot_as_of_utc": self.snapshot_as_of_utc,
...
def observe_news_signal(
    ...
    *,
    ...
    if identifier is not None and (
        not isinstance(identifier, str) or identifier != publication_id(snapshot)
    ):
        raise NewsError("news-signal publication ID mismatch")
    status = str(snapshot.get("run_status") or "")
    signal_text = str(snapshot.get("signal_as_of_utc") or "")
    snapshot_text = str(snapshot.get("snapshot_as_of_utc") or "")
    signal_at = _parse_utc(signal_text, label="signal_as_of_utc")
    snapshot_at = _parse_utc(snapshot_text, label="snapshot_as_of_utc")
    if snapshot_at < signal_at:
        raise NewsError("snapshot_as_of_utc precedes signal_as_of_utc")
    analysis = snapshot.get("analysis")
    assets = analysis.get("assets") if isinstance(analysis, Mapping) else None
    signal = assets.get(key) if isinstance(assets, Mapping) else None
    if not isinstance(signal, Mapping):
        raise NewsError(f"news-signal snapshot lacks {key} aggregate")
    direction = signal.get("direction")
    impact = signal.get("impact")
    confidence = signal.get("confidence")
    horizon = signal.get("horizon_hours")
    change = str(signal.get("change") or "")
    ...
    if (
        ...
        or change not in SIGNAL_CHANGES
        or not isinstance(drivers, list)
        or len(drivers) > 5
        or any(not isinstance(item, str) or not item for item in drivers)
    ):
        raise NewsError(f"news-signal {key} aggregate is invalid")
    confidence_value = _assert_confidence(
        confidence,
        label=f"news-signal {key}.confidence",
    )
    decision_at = as_of.astimezone(timezone.utc)
    age = (decision_at - signal_at).total_seconds()
    available = decision_at >= snapshot_at
    usable = (
        status in ("published", "no_new_evidence")
        and available
        and 0 <= age <= horizon * 3600
    )
    reason = (
        "fresh"
        if usable
        ...
        if age > horizon * 3600
        else f"run_status:{status or 'missing'}"
    )
    return NewsSignalObservation(
        symbol=key,
        run_status=status,
        signal_as_of_utc=signal_text,
        snapshot_as_of_utc=snapshot_text,
    ...
    for index, event in enumerate(events):
        ...
        if isinstance(event, dict) and set(event) == ACTIVE_EVENT_KEYS:
            if previous is None:
                ...
                event["last_material_change_utc"] = _utc_iso(as_of)
            else:
    ...
    for symbol in ASSET_SYMBOLS:
        ...
        if isinstance(raw_drivers, list):
            signal["drivers"] = [
                ...
                if isinstance(driver, str) and driver in active_ids
            ]
