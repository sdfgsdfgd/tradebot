...
| [A04](#xsp-archive-a04-evidence-registry) | Full append-only evidence registry E-000…E-191 |
| [A05](#xsp-archive-a05-decision-journal) | Full frozen decision journal D-001…D-161 |
...
| E-178 | 2026-07-27 05:11 AEST | 1/2/3 | Pre-outcome Opening Edge selected-shadow bridge | implementation `6d87482`; canonical candidate/benchmark/shadow/CLI owners; q and Mac `db/calibration/xsp_selected_shadow.json` | selection `9fac460ea6911b4132188d2ddf5326c9df8f4ce148d1dbdb097deeed80f1abb5`; receipt `faf294f7…`; crown config `bbb0a391…`; full `844 passed, 4 deselected` | The profitability path previously accepted only selected equity derived from a future option-parity recommendation, so Monday's already-frozen directional crown could emit candidate economics but could not start the requested `24h → 48h → five-session` proof. One generic selection boundary now also accepts the exact content-addressed Opening Edge recommendation, rejects identity drift, and projects the unchanged normal-engine candidate ledger into selected equity. Before any Monday price/quote outcome and while the calibration ledger was absent, q atomically froze the exact strategy/config/risk identity; Mac holds a byte-identical receipt. The CLI loads and validates it before broker construction. The first exact `EVALUATED` checkpoint may start evidence coverage; selection itself does not. Broker strategy remains `NO_TRADE`, `order_authority=none`, no live XSP fill is fabricated, and future option-parity selection compatibility remains unchanged. |
| E-179 | 2026-07-27 AEST | 1/2/3 | Complete selected-run history through the weekly deadline | canonical XSP shadow history window and profitability clock | `1 W → 2 W`; run start `2026-07-27 09:30 ET`; weekly evidence deadline `2026-08-03 09:37 ET` | A one-week rolling request evaluated exactly seven days later begins at `09:37` on the prior Monday, omitting the selected run's first `09:30/09:35` evidence and risking a different cumulative replay at the weekly deadline. The shadow window is now two weeks: one bounded five-minute request that retains the entire selected run plus warm-up through the deadline. No crown signal, admission, lifecycle, economics, selected identity, clock state, broker state, or order authority changed. |
| E-180 | 2026-07-27 AEST | management/runtime | Final TICK scope minimization | canonical market, contract-identity, client-history, and XSP management owners | zero consumers of deleted reducer; signed-value ingestion retained | The observation-only market-breadth reducer had no consumer after the XSP breadth challenger was retired; its only caller was its own test. It was therefore deleted rather than preserving a dead research API inside the canonical market engine. The minimum durable core is now exact TICK contract identity plus signed-value historical ingestion, which prevents legitimate negative TICK values from being discarded without creating a breadth subsystem. No XSP fetch, signal, gate, selector, shadow field, receipt, UI, broker request, or order authority remains. |
| E-181 | 2026-07-27 AEST | 2 | P&L-conditioned inverse-source lifecycle rejection | unchanged Opening Edge v1 tape/engine; temporary research monkeypatch only | recent crown `+10.98/17`, loss-only flip `-1.26/17`; one-year crown `+131.74/204`, loss-conditioned handoff `+129.10/200` | Two previously untested but bounded ownership hypotheses were challenged without adding a runtime knob. Permitting an inverse-source exit only while the open trade was non-profitable destroyed recent PF (`1.950 → 0.900`) and worsened drawdown (`7.83 → 10.21`). Always honoring the inverse exit while carrying the opposite re-entry only from a non-profitable trade was recent-identical, then blocked four one-year handoffs and weakened net, PF, drawdown, daily confidence, and cadence. Both stopped at their first discriminating gate; neither opened five years. No source, crown, selection, clock, broker, or order authority changed. |
| E-182 | 2026-07-27 14:00 AEST | 1/3 | Natural Monday producer start, Gateway-token outage, and process-level self-heal | q `tradebot-xsp-quotes.{timer,service}`; on-demand tunnel; Mac IB Gateway; `db/quotes/XSP/2026-07-27.jsonl` | trigger `10:15 AEST`; recorder/tunnel `NRestarts=0`; `197` bounded retries; API auth/listener restored `13:59`; rows `14:00/14:05/14:10`; GTH; stable `42` contracts; two-sided `12/12/29` | The persistent timer fired exactly and the recorder remained alive, but the Gateway's `07:45` process auto-restart did not restore its API listener because of a token fault. The recorder correctly treated repeated resets/timeouts as ambiguous, retried with a bounded 60-second delay, and neither cached absence nor exited. After the user repaired the Gateway token at `13:59`, the existing process reconnected without a service restart or manual capture. Three exact five-minute rows preserve the same qualified universe while delayed two-sided coverage improves; subscription errors remain explicit provenance. The missing `10:15..14:00` prefix is unavailable and must never be reconstructed from later quotes. This proves process-level recovery and initial cadence while leaving full-session completeness and later shadow profitability unproved. Broker/order strategy remains `NO_TRADE`; both services retain `IBKR_READONLY=1`; order authority remains `none`. |
| E-183 | 2026-07-27 15:15 AEST | 2/3 | Actual selected-checkpoint completion-time integrity | canonical `tradebot/research/{live_calibration,xsp_shadow,xsp_shadow_cli}.py`; `tests/test_live_calibration.py` | sources `359ac258…` / `bf4c6a97…` / `7e777001…`; focused `58 passed`; full `844 passed, 4 deselected`; calibration owner `999` lines | The selected profitability contract froze a `90s` evidence tolerance, but the live adapter wrote invocation time into both `evaluation_as_of_utc` and `recorded_at_utc`; a slow result could therefore appear timely. Live checkpoints now retain their slot/evaluation timestamp and separately record actual completion, while the CLI evaluates profitability at completion. Both complete-session and selected-profitability coverage require evaluation and recording to lie inside the same frozen slot tolerance. A deterministic `91s` recording delay is rejected as `INVALID_EVIDENCE`, zero complete sessions, and broken coverage; normal jitter remains valid. Opening Edge signals, trades, economics, selected identity, timer schedule, recorder, broker state, and `order_authority=none` are unchanged. |
| E-184 | 2026-07-27 15:36 AEST | 2 | Source-consistent inverse-quality challenge | q evidence `/home/x/Desktop/tradebot-evidence/XSP/2026-07-27/research/`; unchanged Opening Edge v1 normal engine | artifact `a090b364…`; campaign `48944c1f…`; `36→2→2` identities in `17.1s`; exact control `+120.59/1,019`; sign-consistency `+121.47/1,018` | A frozen `3×4×3` family tested whether coherence, instantaneous direction strength, and retrace should confirm each raw persistent-source reversal before it can drive either entry or exit. The true no-quality control reproduced every crown metric. Every material confirmation failed the recent cadence/economics gate; only the minimal rule that rejects a smoothed reversal when the instantaneous raw score already has the opposite sign survived. It was identical recently and over the latest year, then changed four five-year paths: PF `1.1705→1.1720`, drawdown `59.95→60.26`, and annual deltas `0/0/-0.31/+0.06/+1.13/0` through 2021–2026. It worsened 2023 and removed one 2025 trade, so no stable causal improvement, candidate, source revision, runtime knob, selector, clock, broker call, or order authority was earned. |
| E-185 | 2026-07-27 15:40 AEST | 1/2 | Monday GTH prefix provenance and preregistered research interpretation | first `20` rows of q `db/quotes/XSP/2026-07-27.jsonl`; immutable prefix SHA `9c861fae…`; canonical strict observer plus read-only replay | `14:00..15:35 AEST`; one chain fingerprint; `42→50` retained contracts; strict `30s` usable `1/20`; `330/360/420s` plateau `15/20`; forced model-only `360s` usable `10/20` | The recorder is healthy, but execution and research freshness are different contracts. Delayed option ticks arrive roughly one five-minute capture old. In ten early rows IBKR refreshed the XSP ticker timestamp while repeating `last=741.29`; option-model consensus was `746.37..747.10`, so last-only cash was economically stale by `5.08..5.81` points despite looking fresh. Forcing GTH/Curb to qualified option-model consensus and retaining the existing `0.05` dispersion veto leaves ten honest usable observations, whose parity path rises from about `746.495` to `747.015`; the rest fail on pair count or dispersion. Before complete `2h/4h/6h` outcomes existed, the research replay was frozen at `360s` (one capture plus bounded jitter), model-only GTH/Curb anchoring, existing pair/dispersion gates, and zero direction/order authority. Strict `30s` execution policy and first-seen forecast context remain unchanged; implementation waits until the active recorder exits. |
| E-186 | 2026-07-27 16:45 AEST | 1/2 | First authentic model-only two-hour GTH option path | q evidence `tradebot-evidence/XSP/2026-07-27/quotes/gth-prefix-34.jsonl`; immutable first `34` live rows; prefix SHA `bec21d16…`; local preregistered v2 interpretation `e5fd00d2…` / `f89d397f…` / `7b6ffdd7…` | `34` ordered GTH rows; one chain; one expiry `20260727`; `42→50` retained contracts; strict `2/34`; model-only `16/34`; exact pair `00:35:06→02:35:16 ET`, `7209.14s`, five pairs each, ages `296.83/299.81s`, `746.495→748.170`, `+1.675` points | The preregistered `xsp.preopen-option-path.v2` contract produced its first real horizon without widening age, pair, dispersion, expiry, or causal-anchor rules. Both endpoints use qualified same-expiry option-model consensus; the nine-second schedule jitter remains far inside the existing seven-minute anchor tolerance. This proves only the `2h` observational mechanism on an immutable live prefix. It does not prove `4h/6h`, the final `09:20..09:30 ET` boundary, execution freshness, alpha, a selector, a gate, or order authority. The exact prefix is preserved read-only from the growing tape at mode `0600`, owner `x:x`; the q recorder remains pinned and untouched. Local v2 is committed but remains unpushed and undeployed until the complete session exits. |
| E-187 | 2026-07-27 17:15 AEST | 1/3 | First natural interval-normalized causal-news transition | q `tradebot-news.{timer,service}`; append-time `history/2026-07.jsonl`; local canonical `xsp_fundamental_context_at` | service `17:07:12→17:15:15 AEST`, success/no retry; publication `a37f8e3f…`; latest `2b9571d…`; history `b790c5fb…`; `14` publications; XSP `-1/66/.95/24h`; pressure `-0.627`; delta `+0.0285/14,979s`; velocity `+0.00685/hour` | The first natural publication after freezing level/change/interval semantics completed inside the corrected fourteen-minute inference boundary and preserved the prior durable files until atomic publication. XSP remains directionally bearish while the reducer marks pressure weakening. The numeric easing impulse slowed from the prior two transitions (`+0.014014/hour`, then `+0.009204/hour`, now `+0.00685/hour`), distinguishing a persistent bearish prior from a fading de-escalation impulse. No contemporaneous XSP outcome is yet available, so this is neither a bullish pivot nor evidence for a multiplier, selector, veto, hold, size, strategy change, or order authority. The append-time record is sufficient for later exact TA/news/volatility joins without narrative persistence in trading receipts. |
| E-188 | 2026-07-28 00:41 AEST | 1/2/3 | Opening Edge v2 balanced 24/5 research coronation | `backtests/xsp/opening_edge_v2_balanced_24x5.json`; `backtests/xsp/current-lf.json`; `backtests/xsp/leaderboard.md`; centralized RTH admission and GTH cascade owners | crown `f879cc20…`; combined ledger `5af0c8b2…`; RTH/GTH ledgers `1c511d2f…` / `9e04807f…`; recent `+11.6675/16`; annual `+15.6348/+29.3083/+134.7224`; three-year `+179.6656/725`, PF `1.4562`, DD `26.4689`; publication `b8defd8`; full `861 passed, 4 deselected` | The user explicitly crowned the balanced all-hours successor after it remained positive in recent, every July-to-July annual slice, both directions, both lanes, and aggregate three-year evidence above the cadence floor. RTH retains the XSP-native multitimeframe opening-edge owner plus scale-free bullish exhaustion/maturation filters; GTH uses the SPY fast-to-slow velocity cascade, down-only admission, up evidence for exits, and flat-by-09:25 lifecycle. Both policies now live in shared runtime owners and reproduce every frozen trade, side, price, reason, and P&L without a research-only projection. The machine-current declaration and Bot UI expose named SPY shadow leaves, while the artifact remains historical research authority only. V1 lineage and the aggressive challenger remain immutable in the leaderboard. No v2 selected run, profitability clock, broker submission, order, or capital authority was created. |
| E-189 | 2026-07-28 01:35 AEST | 1/2/3 | Exact Opening Edge v2 SPY transport and execution-economics rejection | `backtests/xsp/opening_edge_v2_spy_execution_receipt.json`; canonical backtest engine and `tradebot/research/xsp_opening_edge_v2.py`; IBKR one-share SPY what-if | receipt `dfd90c4e…`; raw audit `7a19334c…` / `e4a88d28…`; crown `f879cc20…`; exact XSP-signal/SPY-fill frozen-cost `+178.48/725`, PF `1.4572`, DD `26.64`; warm-prefix `+178.29/725`; measured-cost `-758.80/413`; full `864 passed, 4 deselected` | A same-resolution engine defect silently replaced an explicitly supplied SPY execution tape with the XSP signal tape whenever both were five-minute bars. The shared run/summary boundary now preserves every explicit execution tape and uses the signal tape only when execution is omitted. With genuine SPY fills, all three annual slices, recent, both directions, and both lanes remain positive under the frozen `$0.10` research contract, proving signal portability. The broker preview qualified SPY `756733`, observed `0.03` spread, and returned up to `$1.01540245` commission per side; the resulting `2.0608049`-point round trip overwhelms the one-share edge and exhausts the USD `1,000` reference before the three-year tape ends. One content-addressed owner now binds the current crown, XSP session clock, XSP/SPY signal provenance, SPY fills, causal warmup/no-backfill start, paired research/broker equity, and a fail-closed `HOLD`. No v2 selection, profitability clock, q deployment, broker submission, order, or capital authority was created. |
| E-190 | 2026-07-28 01:57 AEST | 1/2/3 | Preflight- and restart-safe Opening Edge v2 broker observer | `tradebot/research/xsp_opening_edge_v2.py`; `xsp_shadow_cli.py`; `deploy/systemd/tradebot-xsp-shadow.{service,timer}`; `tests/test_live_calibration.py` | owner `f3f0af1f…`; CLI `384beea8…`; service/timer `921e5a65…` / `814f3726…`; full `868 passed, 4 deselected`; q unit verification clean; q still pinned to `c93bc63…` with active recorder | The paired v2 owner now requires its first content-addressed checkpoint before the exact `20:15 ET` GTH start, rejects a late first invocation, off-boundary start, restart drift, or crown-config drift, and uses prior history only for causal warmup/reconstruction. One explicit CLI mode ignores the obsolete v1 selection, requests SPY full-session execution plus XSP RTH signal history, preserves XSP RTH provenance during GTH restarts, logs compact timestamp-valid fundamental pressure, and records research/broker equity separately. Any missing XSP provenance remains a visible SPY fallback but is `STALE_DATA`, never exact coverage. The bounded timer covers completed GTH/RTH bars and one `16:17 ET` finalization read; Curb has no trade ownership. All receipts retain `order_authority=none`; measured-cost `HOLD`, `NO_TRADE`, absent v2 selection, and `NOT_STARTED` milestone clocks remain unchanged. The candidate units were syntax-verified on q without installation; deployment still waits for the active recorder's terminal receipt. |
| E-191 | 2026-07-28 02:48 AEST | 1/2/3 | Anchored v2 observer and fixed-transport impossibility proof | `tradebot/research/xsp_opening_edge_v2.py`; shared `engines/market.py` exit clock; backtest/live consumers; `backtests/xsp/opening_edge_v2_spy_execution_receipt.json` | projected `117,433` GTH bars; frozen tape `6d239c88…`; canonical SPY-fill ledger `04987ff5…`; parity `ad220ece…`; exact `725` trades / `+178.48` / PF `1.4572` / DD `26.64`; measured RT `2.0608049`; hindsight cadence ceiling `1.221649` | The published observer had mistakenly sent raw SPY prices into a GTH sensor researched on SPY returns scaled from the last exact completed XSP RTH close. One deterministic projector now requires that causal anchor, ignores same-day future closes, preserves raw SPY only for execution, and returns `NO_DATA` when the anchor is absent. A second hidden mismatch compared a civil `09:25` clock numerically against Sunday-evening bars and could flatten a new GTH position immediately; one shared session-date predicate now governs backtest and live. The corrected projection reproduces every frozen GTH bar and all `179 GTH + 546 RTH` SPY-fill economics. Ten terminal reasons change from harness-incidental `close_eod` to artifact-explicit `exit_time` with identical times, sides, fills, and P&L. At measured one-share costs the full tape is `-1243.10`; even hindsight selection of the best `479` trades required for `160/year` loses `-401.96`, with zero same-timestamp transitions available for fee netting. This supersedes E-190's stale raw-SPY fallback: exact XSP anchoring now fails closed. q remains untouched on pre-v2 source while its recorder runs; `NO_TRADE`, no v2 selection, `NOT_STARTED`, and zero order authority remain unchanged. |
| E-192 | 2026-07-28 07:41 AEST | 1/2/3 | Authentic fixed UPRO/SPXU transport rejection and clean recorder-to-fetch handoff | frozen gate `4da254ea…`; tracked `opening_edge_v2_upro_spxu_acceptance_receipt.json`; q terminal tape/fetch receipts; authenticated ETF cache and exact cash replay; immutable q archive | tape `196` rows / `05fb7846…`; fetch `cf77ce86…`; UPRO `211,679` bars / SPXU `210,698`; archive `21` files / `21f1620b…`; balanced `116` invalid GTH SPXU fills; RTH `546/546` lifecycle parity; Fixed T+1 `210` executed / `472` blocked / `-$583.69` / PF `0.344` / intrabar DD `$588.64` | The recorder exited successfully with complete RTH/Curb coverage and an explicitly censored early-GTH prefix, then the dependency-owned hydrator fetched authentic three-year UPRO/SPXU history. Its first two attempts were preserved as useful operational failures: the one-shot did not own the on-demand Gateway tunnel, then q Python lacked IBKR's `US/Eastern` timezone alias. Requiring the tunnel and pinning `tzdata` produced a clean retry. Balanced 24/5 is ineligible because `114` selected GTH SPXU timestamps have zero volume and two split-day timestamps are absent. The unchanged RTH fallback retains exact signal/fill ordering but strict settled cash admits only bullish UPRO trades, loses materially, and fails cadence, attribution, PF, and drawdown; scheduled-next-open and conservative Tiered variants also fail. The exact tape, failures, fetch/cache, replay, verdict, and sources are redundantly archived read-only on q. The frozen verdict is `HOLD`: no selected v2 shadow, profitability clock, broker submission, order, or capital authority exists. |
| E-193 | 2026-07-28 10:22 AEST | 2/3 | First frozen Opening Edge v2 GTH checkpoint | q `bb5bcd8…`; canonical shadow service; ledger; compact news state; preregistered audit; immutable first-evaluation archive | checkpoint `4dc8695a…`; ledger `5353a70e…`; selection `faf294f7…`; SPY age `120.485612s` / `2,859` complete bars; XSP `780` bars; `1,405` GTH signal bars; archive manifest `76651618…` | The one-shot fired only after the frozen `20:15 ET` start and appended exactly one `EVALUATED/GTH` checkpoint at `20:22:00.485612 ET`. Exact XSP anchor provenance and SPY history were fresh, both paired cost profiles reproduced stable run identities, and no position, closed trade, P&L, or safety breach existed. The checkpoint causally recorded only compact XSP news pressure `-1/70/.97/24h`, signed `-0.679`, delta `-0.0838` over `14,919s`, velocity `-0.020221/hour`. The first audit deliberately failed when it demanded unrounded velocity although the canonical reducer stores six decimals; that failed output was preserved, the audit expectation was corrected to the existing storage contract, and every remaining assertion passed. The v1 selection stayed byte-identical, execution eligibility remained `HOLD`, and order authority remained `none`. |
| E-194 | 2026-07-28 10:27 AEST | 2/3 | Recurring observer restart/cadence proof and SPY entitlement boundary | canonical recurring timer/service; second ledger checkpoint; broker read-only entitlement probes; immutable recurring archive | checkpoint `aad9021b…`; ledger `d6dc2a5e…`; selection `faf294f7…`; SPY age `120.702086s`; recurring manifest `d8509c3f…`; first archive manifest `76651618…` | Only after E-193's archive verified did the canonical non-persistent timer become enabled. Its first recurrence landed at `20:27:00.702086 ET`, preserved the same frozen start, crown config, research/broker profile IDs, source ownership, compact news fingerprint, zero-position/zero-trade state, and zero safety breaches, then scheduled the next exact cadence. SMART/ARCA SPY streaming returned broker error `10089` and no quote because API top-of-book needs additional entitlement; no subscription, agreement, fee, or snapshot was accepted. Explicit `OVERNIGHT` SPY independently returned market-data type `1` and a sane live `738.98/739.13` book after the venue opened. That streaming fact is separate from history: pre-open historical bars lagged roughly ten minutes, while the live GTH history used by both canonical checkpoints was fresh. Recurrence proves observation only; execution remains `HOLD`, `NO_TRADE` remains selected, and the profitability clock remains `NOT_STARTED`. |
| E-195 | 2026-07-28 10:51 AEST | 2/3 | State-change-only UPRO/SPXU execution evidence owner | published `90ea30c9a81c39090b56d2f58e0cee64861b8c21`; `tradebot/research/xsp_execution_observer.py`; CLI integration; focused/capability/full verification | owner `dd8ebc71…`; CLI `fd0f9459…`; tests `abb1de69…`; frozen gate `4da254ea…`; frozen HOLD receipt `6d9fc840…`; full `876 passed, 4 deselected` | One cohesive research owner now compares the exact paired-v2 position state and performs no market-data or broker work while flat or unchanged. A first entry snapshots one BUY; an exit one SELL; a flip the prior SELL plus new BUY. Each leg requires broker-qualified STK identity, fresh streaming NBBO and age, uses the shared tick-normalized `OPTIMISTIC→MID→AGGRESSIVE→CROSS` price owner, and captures only a non-transmitting what-if preview. Research/broker state drift, persisted-state mismatch, or frozen gate/receipt tampering fails closed. Every record retains the rejected UPRO/SPXU `HOLD`, `selected=false`, `submitted_orders=0`, stopped profitability clock, and `order_authority=none`. The first test exposed and repaired a genuine idempotence defect caused by comparing transport-enriched persisted state with raw signal state. q remains pinned at `bb5bcd8…` until its active recorder exits, so no imported source changed beneath that producer. |
...
| D-149 | Preselect the unchanged research crown for synthetic profitability attribution before Monday outcomes | Requiring five future option-parity sessions before any selected equity could exist made the requested Monday directional `24h → 48h → week` proof impossible and would invite outcome-aware selection. Freezing the exact Opening Edge strategy/config/risk identity before evidence solves attribution without changing its engine, broker state, or authority | A future content-addressed selection with stronger complete prospective evidence explicitly supersedes this shadow run while no position is open; broker/order promotion still requires the full milestone ladder |
| D-150 | Retain two weeks of shadow history for the seven-day proof | The selected run starts Monday `09:30`, while a one-week request at the following Monday `09:37` drops its first seven minutes and can no longer reproduce the frozen cumulative path. Two weeks is the smallest standard IBKR duration that preserves both run and warm-up through the deadline without introducing incremental state or a large historical request | Replace rolling replay with a durable session-accumulation state only after it can preserve exact normal-engine and restart parity beyond this initial weekly quest |
| D-151 | Retain signed TICK ingestion, not a dormant breadth API | Correctly accepting negative TICK values is a generic data-integrity rule. The unconsumed breadth reducer, however, had no Monday or XSP authority and preserved an abandoned experiment in a canonical engine | A future breadth study must preregister independent evidence and add the smallest reducer at its research boundary; it must not revive implicit XSP runtime authority |
| D-152 | Close P&L-conditioned inverse-source ownership | Loss-only exits failed immediately; flattening profitable flips while reversing losing flips was recent-neutral but weaker over one year. Extending either to five years or adding a permanent knob would spend complexity on a dominated mechanism | Reopen flip ownership only when timestamp-correct prospective liquidity, causal news, or path-state evidence identifies a falsifiable subset before outcomes |
| D-153 | Require actual checkpoint completion inside the frozen evidence slot | Invocation time proves when work began, not when its cash history, candidate equity, and reconciliation became durable. Reusing it as recording time let a slow checkpoint satisfy the `90s` tolerance without evidence | A stronger transactional scheduler receipt may bind trigger, start, broker completion, ledger fsync, and service exit while preserving the same fail-closed deadline |
| D-154 | Close historical inverse-source quality tuning | Coherence, aligned instantaneous strength, and retrace confirmation were tested with one source-consistent owner and exact no-quality control. Material filters failed recent evidence; the only surviving sign check changed four five-year paths, worsened 2023 and drawdown, and deleted one trade for `+0.88` total points. Encoding it would add policy without a stable mechanism | Reopen only when first-seen prospective news, option liquidity, or path evidence preregisters the subset before its outcomes and improves the unchanged crown through the milestone ladder |
| D-155 | Separate strict execution freshness from a versioned pre-open research replay | GTH option ticks are normally one capture old, while IBKR can refresh an unchanged XSP cash-last timestamp. Relaxing the shared `30s` observer would mislabel stale execution evidence; leaving research strict would discard valid long-horizon path information. Freeze `360s`, option-model-only GTH/Curb anchoring, and existing dispersion/pair vetoes as a separate observation contract before complete horizons exist | The completed prospective tape falsifies the `330..420s` plateau, exact model anchoring, or causal 2h/4h/6h completeness; any replacement must be preregistered as another version and cannot rewrite first-seen records |
| D-156 | Treat news impact as a prospective pressure coordinate before estimating influence | `direction × impact / 100` preserves sign and the reducer's calibrated shock scale, but is not a percentage of XSP return or causal attribution. Selected evaluation logs retain only `usable`, signal time, snapshot fingerprint, direction, impact, and confidence; no news markdown, narrative, mechanism, headline, or driver dump. Missing/stale remains unavailable, and long-reluctance versus short-patience stays separately attributed with unchanged size and crown ownership | Only timestamp-correct prospective pairs across materially different news states may fit a coarse hysteretic influence policy; it must improve expectancy/adverse selection without harming reversals, cadence, or safety before a later version can receive any trade authority |
| D-157 | Treat confidence-weighted pressure, change, and interval-normalized velocity as volatility-conditioned observational coordinates | `direction × impact / 100 × confidence` preserves the reducer's signed magnitude while discounting uncertainty; consecutive usable snapshots expose strengthening, weakening, or reversal without interpreting impact as percent price causation. Because publication spacing is not uniform, every delta carries its exact interval and per-hour velocity; a fixed delta over 45 minutes is not equated with the same delta over four hours. During the `24h→48h→week` proof, join only compact timestamp/fingerprint/direction/impact/confidence/freshness/horizon state to realized volatility, directional impulse, entries/flips/exits, MFE/MAE, and net P&L. Missing, stale, failed, or expired news means unavailable; no backfill, narrative dump, constant `40–60%` weight, selector, size, or order authority | Prospectively frozen TA-only versus TA-plus-news cohorts across materially different level/velocity/volatility states improve expectancy, adverse-selection avoidance, or drawdown without harming cadence or reversal capture; only then may a later version preregister one centralized toggleable conservative permission/hold/veto policy |
| D-158 | Crown balanced v2 historically but require a new execution-qualified selection | The balanced 24/5 candidate materially improves v1 across the chosen three-year annual-slice, aggregate, cadence, and drawdown criteria, and exact centralized parity makes it the rightful research crown. Its GTH history remains SPY-derived and its selector maps a non-tradable XSP signal reference to a whole SPY share; neither historical `$0.10` friction nor v1's selected receipt proves that transport. V2 therefore cannot inherit v1 attribution or start a prospective clock | Exact one-share signal/fill tracking, commission, spread/slippage, availability, and restart evidence remains net-positive under the unchanged v2 identity, after which a new content-addressed selected-run receipt is frozen before outcomes |
| D-159 | Separate signal transport from executable fixed-unit economics | Genuine SPY fills preserve Opening Edge v2's directional edge under its frozen research cost, but a whole SPY share at the measured IBKR commission/spread loses roughly two points per round trip and exhausts the capital reference. Historical portability therefore cannot authorize selection, start a profitability clock, or be relabeled as broker profitability. A paired non-submitting observer may continue to measure signal and cost attribution, but `NO_TRADE` remains authoritative | An unchanged fixed-unit transport passes authentic spread, commission, availability, tracking, cadence, drawdown, and net economics before a newly frozen v2 identity opens any prospective milestone clock |
| D-160 | Freeze the v2 observer start before outcomes and reuse one explicit service/ledger path | Deriving a run start after an invocation would silently backfill an already-observed GTH prefix; recomputing an earlier RTH lane from SPY after a GTH restart would silently lose exact crown provenance. The existing one-shot therefore gains one explicit v2 mode: it freezes the next untouched `20:15 ET` boundary before broker work, reuses the same immutable start/config on every restart, retains XSP RTH history beside SPY full-session execution, and marks any exact-RTH fallback stale. V1 selection, profit clocks, orders, and Curb trade ownership remain absent | A pre-boundary q preflight and first post-close broker invocation reproduce deterministic paired identities across service restart, with fresh exact provenance and no source, start, config, or ledger drift |
| D-161 | Make anchored GTH provenance and 24/5 exit clocks explicit, fail-closed contracts | Raw SPY levels are not the crowned GTH signal tape, and a civil-time `>=09:25` check treats Sunday evening as already past Monday's exit. The observer must project SPY returns from the strictly prior exact XSP close, execute on raw SPY, and compare exit clocks inside the mapped trading date through one shared backtest/live predicate. Missing anchors are `NO_DATA`, not a degraded proxy | A provider supplies authentic continuous XSP-equivalent GTH prints or a cheaper execution instrument while preserving the same causal projection, ordered economic ledger, and session-clock semantics |
| D-162 | Reject fixed UPRO/SPXU and do not let exact signal parity override transport failure | Authentic GTH history cannot support the selected inverse-ETF timestamps, while the exact RTH ledger cannot express both directions under strict settled cash and fails every frozen economic gate. Fabricating fills, weakening liquidity, changing fees after outcomes, or relabeling one-direction partial execution as the balanced crown would destroy attribution. The paired observer may continue with zero order authority, but selection and milestone clocks remain closed | A materially cheaper fixed cash transport passes preregistered authentic liquidity, settlement, both-direction attribution, cadence, PF, drawdown, and period economics before any new selected-run receipt is frozen |
| D-163 | Activate recurring observation only after an immutable first-bar audit | A successful process exit is weaker than source/start/config/freshness/selection/safety proof. The first v2 GTH checkpoint therefore remained a one-shot with recurrence disabled until its complete before/after ledger, selected state, news state, units, journals, and audit were content-addressed. The later timer activation cannot grant selection, execution, order, or profitability-clock authority | A stronger transactional scheduler may bind trigger, dependency startup, broker completion, ledger fsync, archive receipt, and recurrence in one equivalently fail-closed owner |
| D-164 | Keep broker streaming entitlement and historical-bar freshness as separate provenance facts | SMART/ARCA top-of-book is not currently API-entitled, explicit OVERNIGHT streaming is live, and historical availability changes across the GTH open. Treating any one of those facts as authority for the others would either discard valid live GTH evidence or silently call delayed history real time. The canonical observer continues to enforce its own completed-bar age; streaming evidence remains separately attributed | A centralized broker-qualified live-tail recorder proves exact completed-bar equivalence and restart behavior without subscriptions, synthesis, timestamp drift, or strategy changes |
| D-165 | Observe rejected ETF transport only on actual paired-v2 position-state changes | Polling UPRO/SPXU every five minutes would duplicate the canonical cadence, inflate the ledger, and turn a failed historical candidate into ambient architecture. One state-change owner instead reuses shared contract, quote-health, tick, ladder, and broker-preview semantics; flat/unchanged state is silent, profile or artifact drift fails closed, and every observation retains the frozen `HOLD`, zero selection, zero clock, and zero order authority | A content-addressed prospective state-change cohort demonstrates executable pricing and materially cheaper realized friction across enough independent transitions to justify a separately preregistered transport challenge; observation alone cannot reopen the historical verdict |
