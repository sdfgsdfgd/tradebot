...
24. `[WIP]` Sharpen the MCL Two-Speed Auction Relay through the user-authorized
    ...
    Active v13 keeps that signal fixed and tests one failed-auction-only profit
    memory: arm after `0.5%` favorable excursion, then retain `25%` of MFE from
    the next one-minute bar; zero/50% retention are sensitivity neighbors.
