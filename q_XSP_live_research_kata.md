...
16. `[WIP]` Qualify the already-positive v3 UPRO/SPXU RTH path under the
    ...
    One dated q timer now invokes that owner at `09:39 ET` only after the
...
| E-300 | v3's first prospective loss is now causally event-attributed without a hindsight veto. The `11:00 ET` long and `12:15 ET` exit preceded the scheduled July 29 FOMC release by `180m/105m`; q's natural `12:17:49 ET` news state then strengthened bearish pressure to `-0.8428` with delta `-0.1132`. The official `14:00 ET` statement held `3.50–3.75%` by `9–3`, with three voters preferring a `25 bp` hike ([Federal Reserve](https://www.federalreserve.gov/newsevents/pressreleases/monetary20260729a.htm)). At `14:07 ET`, after evaluating the complete post-release bar through `14:05`, v3 remained flat: source state `down`, no qualified turn/proposal/admission, and simulated net unchanged at `-2.42`; broker positions/orders remained zero. The user's contemporaneous `36–40%` hike-probability observation is retained as unverified context, not a strategy fact. This creates a prospective scheduled-event volatility/ownership cohort only—no date whitelist, news-direction authority, strategy edit, selection, order, or clock |
