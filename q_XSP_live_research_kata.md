...
| XSP pivot challenger | Dedicated [`q_pivot.md`](q_pivot.md) frontier `0425183e…` freezes one causal phase-front program. P-001 landmark preregistration `07506749…` binds the exact `184,957`-bar / `754`-session tape, prior-21-session scale, `0.25×` causal confirmation, `0.50×` serious outgoing wave, and diagnostic-only neighbors before landmark outcomes. P-002's physically separated oracle/causal owners pass seven synthetic paths; their content-addressed generation must now freeze before the count-only tape census. Timing-only challenger comes first, a complementary missed-pivot sleeve opens only if required, and GTH qualifies independently. It has no live authority |
