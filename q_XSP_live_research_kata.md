...
- **Status:** `[ACTIVE] Opening Edge v3 is the frozen historical crown. Its
  first Tiered UPRO/SPXU RTH round trip is terminal and q is flat: v3
  conditionally surrendered `down` without admitting `up`, then the live owner
  sold all `23 SPXU`. The original prefix remains invalid; the recurring timer
  is paused until one clean post-flat proof identity is frozen. The 24/5 GTH
  lane remains observation-only`
...
| Selected live run | continuity successor `ab0bb43a…`; selection file `be949517…`; predecessor `d0726236…` is archived intact. This identity managed the inherited position and is not a clean profitability-prefix start |
...
| Runtime source anchor | q is clean at published `69e489b…`; central owner `643b6a…`, crown `d47eb39…`, canonical daily-context fingerprint `7a139df1…` |
| Observer cadence | Five-minute timer paused after clean post-fill restart proof. Strategy truth is flat: the `12:00 ET` source reversal exited `down` at `12:05 ET` next-open, but `entry_dir=null` correctly queued no `up` re-entry |
...
| Profitability clock | `NOT_RESTARTED`. Original prefix is permanently `INVALID_EVIDENCE`; continuity run `ab0bb43a…` proves position-safe migration/exit but began with an inherited holding. No missing slot or inherited P&L may be backfilled into the clean 24h prefix |
...
| Next irreversible evidence | freeze one clean post-flat immediate-proceeds selection, then resume the five-minute owner and anchor new 24h → 48h → five-session receipts from its immutable start |
| First active work | prove the clean post-flat selection/restart boundary, then resume 15-minute evidence reviews over every proposal/veto, slope/velocity/ATR and daily context, fill/cost, MFE/MAE, exit, safety, and TA/news contradiction |
...
20. `[WIP]` Freeze a clean post-flat immediate-proceeds identity. Preserve the
    original prefix as invalid and the continuity run as migration evidence;
    only the new immutable flat start may anchor 24h, 48h, and five-session
    verdicts.
...
Tiered UPRO/SPXU RTH cash authority is now flat under continuity selection
`ab0bb43a…`. Its first qualified bearish transition bought `23 SPXU @ $39.10`;
v3 later surrendered `down` without admitting `up`, and the shared ladder sold
all `23 @ $38.56`. Total commission was `$0.659007`; realized P&L was
`-$13.079007`. Post-fill restart agrees on `SPXU=0`, `UPRO=0`, no open order,
no duplicate, `$1,318.051` settled USD, and no safety breach. GTH signal
...
the fail-closed repairs left three scheduled slots absent. The continuity
successor is valid migration/exit evidence but began with an inherited holding;
it cannot start a clean clock. Missing slots and inherited P&L are never
backfilled. A clean 24h → 48h → five-session proof begins only from a flat,
freshly frozen identity.

**Predictive observation:** after one clean post-flat selection/restart proof,
the next legitimate capital event is a newly admitted v3 direction. Fifteen-
minute reviews then compare live P&L, MFE/MAE, gate state, multihorizon
slope/velocity/ATR, daily context, and attribution-only news pressure without
altering direction or forcing an entry.
