...
| XSP pivot challenger | Dedicated [`q_pivot.md`](q_pivot.md) frontier `e4b5ffd2…` freezes one causal phase-front program. P-001 landmark preregistration `07506749…` binds the exact `184,957`-bar / `754`-session tape and three fixed scale profiles. P-002 R2 `77083184…` proves exact `1,360/1,360` terminal-censor integrity and broad `2,641`-landmark morphology, but honestly rejects the primary as a single-threshold oracle: lower serious-neighbor correspondence passes at `73.19%`, higher is `50.09% < 60%`. Economics never opened. The next count-only law is a three-profile consensus core under the same thresholds and five-minute match clock; timing challenger, complementary sleeve, and GTH qualification remain downstream with no live authority |
