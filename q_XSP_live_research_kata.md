...
| Observer cadence | Both five-minute timers are active. The scheduled `16:32Z` Hawkeye checkpoint found both selected runs flat/clean with zero submissions/P&L/DD/breaches. XSP recovered to `759.03`; every `5/15/30/60/120m` slope and velocity was positive, ATR ratio rose to `0.807`, and coherence reached `1.00`, but the fresh five-minute up turn was outside the frozen entry window and produced no proposal/admission. Gold retained its completed `16:00Z` D1/H4-down source with no target while live `1OZZ6` remained type-1 and actionable |
...
- `[INCUBATING 2/6]` **XSP cooldown-edge paired-veto exhaustion:** an
  eligibility-only census reproduced `40708a28…` without reading returns,
  MFE/MAE, or P&L. Down-retrace→up-ATR veto pairs occur `121` times across
  `118` sessions and the mirror occurs `83/80`; the exact three-bar/`15m`
  cooldown edge contains `72+33=105` pairs. At checkpoint `16:32Z`, the live
  post-veto path had recovered to all-positive slopes/velocities and coherence
  `1.00`, yet remained correctly non-actionable outside the window. At `6/6`,
  a preregistered winner may measure residual-session range separately for
  flat versus incumbent states; it cannot reopen an entry or cold-start path.
- `[INCUBATING 2/6]` **Gold reversal-financing maturation:** the existing
  preregistered onset tape already owns `4h/12h/24h` outcomes. Its completed
  `16:00Z` source is the first live specimen combining sharply positive H4
  fast slope/acceleration (`+5.953/+5.839`) and a fresh positive-news reversal
  with D1/H4 ownership still down and UUP/TIP financing adverse. Preserve this
  as a TA-first maturation cohort against matched TA-only controls; do not add
  another observer, read the sealed outcome early, or let news select an entry.
...
| E-375 | scheduled `16:12Z` Hawkeye began the next six-checkpoint block with both canaries flat/clean, `$1,318.05` settled USD, and zero relevant positions/orders, fills/trades, P&L/DD, or breach. XSP's `16:10Z` close remained `758.47`; `5/15/30/60/120m` slopes were `flat/down/up/up/up`, velocities `down/down/down/up/down`, ATR ratio/velocity/acceleration `0.686/+0.000003/+0.009672`, and coherence `0.60`. The retained source was up, but `weak_direction`, a lone `15m` down turn, and `outside_window` produced no proposal/admission. Long context remained `5/10/21/63/84 up, 42 down`, hard/soft up, `aligned_up/high_up`; easing-bearish news stayed observation-only at `-0.432/+0.1403/+0.033576h^-1`. Gold's new completed `16:00Z` H4 source remained raw-down/D1 hard+soft-down age `148` with no turn/proposal/target, while fast H4 slope/acceleration/spread velocity rose to `+5.953/+5.839/+3.410 dollars`; news reversed positive to `+0.1764`, delta `+0.4926`, velocity `+0.117886h^-1`, confidence `0.84`, against adverse UUP/TIP financing. Live type-1 `GCZ6/1OZZ6` parity remained usable at `$0.30/$0.25` spreads. These tensions seed XSP residual-range ownership and Gold reversal-financing maturation at `1/6`; no live authority changed |
| E-376 | scheduled `16:32Z` Hawkeye kept plan `b4a920b3…` and both canaries flat/clean at `$1,318.05` settled USD, with zero relevant holdings/orders, fills/trades, P&L/DD, or breach. XSP recovered `758.47 -> 759.03`; every `5/15/30/60/120m` slope (`+0.04218/+0.02593/+0.01297/+0.01639/+0.01128`) and velocity (`+0.04481/+0.01538/+0.01209/+0.00925/+0.00226`) was positive, ATR ratio/velocity/acceleration were `0.807/+0.003504/+0.006583`, and coherence was `1.00`. A fresh five-minute up turn remained `outside_window`, so the centralized owner correctly emitted no proposal/admission and transport stayed `flat_target`. Long context remained `5/10/21/63/84 up, 42 down`, hard/soft up and `aligned_up/high_up`; news remained observation-only at `-0.432/+0.1403/+0.033576h^-1`. Gold retained the completed `16:00Z` D1/H4-down source, null turn/proposal/target, positive H4 fast slope/acceleration and positive-news reversal against adverse macro financing; live type-1 `1OZZ6` was actionable at `4092.25/4092.75`, but no synthetic entry was created. Outcome-blind census `900e6e2b…` reproduced XSP ledger `40708a28…` and found `121` down-retrace→up-ATR pairs plus `83` mirrors, including `105` exact `15m` cooldown-edge pairs, while explicitly reading no returns/MFE/MAE/P&L. This narrows the XSP candidate to cooldown-edge residual-range ownership and advances both candidates to `2/6`; no production, strategy, selection, capital, order, or graduation authority changed |
