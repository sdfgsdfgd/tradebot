...
We agreed to implement “Realism v1” (spot-focused) with these knobs:
...
- **D) Costs**: model **spread = $0.01/share each side** (implemented as half-spread on entry/exit) and **commission = $0.00/share** (for now).
...
### 1) Realism v1 implementation (spot only)
- Add config knobs for:
  - entry fill timing (close vs next-open)
  - intrabar PT/SL using OHLC
